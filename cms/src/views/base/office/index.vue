<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-if="checkPermission(['ROLE_OFFICE_SAVE'])" class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">
        {{ $t('table.add') }}
      </el-button>
    </div>
    <el-table
    ...
    <el-dialog :title="$t('table.add')" :visible.sync="dialogAddFormVisible">
      <el-form ref="addForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('manage.parent')" prop="parentId">
          <treeselect v-model="temp.parentId" :show-count="true" :default-expand-level="2" :multiple="false" :options="officeTree" :placeholder="$t('manage.parent')" />
        </el-form-item>
    ...
    <el-dialog :title="$t('table.modify')" :visible.sync="dialogModifyFormVisible">
      <el-form ref="modifyForm" :rules="modifyRules" :model="modify" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('manage.parent')" prop="parentId">
          <treeselect v-model="modify.parentId" :show-count="true" :default-expand-level="2" :multiple="false" :options="officeTree" :placeholder="$t('manage.parent')" />
        </el-form-item>
        <el-form-item :label="$t('manage.office')" prop="name">
          <el-input v-model="modify.name" />
        </el-form-item>
        <el-form-item :label="$t('manage.remark')" prop="remark">
          <el-input v-model="modify.remark" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import { officeList, officeSave, officeStatus, officeTreeList } from '@/api/office'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      officeList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          officeSave(this.temp).then(response => {
    ...
    updateData() {
      this.$refs['modifyForm'].validate((valid) => {
        if (valid) {
          ...
          officeSave(tempData).then(response => {
            for (const v of this.list) {
              if (v.id === response.data.id) {
                const index = this.list.indexOf(v)
                v.parnetId = response.data.parentId
    ...
    handleModifyStatus(id, status) {
      officeStatus({ 'id': id, 'status': status }).then(() => {
        for (const v of this.list) {
          if (v.id === id) {
            const index = this.list.indexOf(v)
            v.status = status
